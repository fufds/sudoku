...
public class SDK {
	...
	public static void main(String[] args) {
		...
		src[5]=new int[] {0,0,3,2,0,0,0,8,0};
		src[6]=new int[] {0,6,0,5,0,0,0,0,9};
		src[7]=new int[] {0,0,0,0,0,0,0,0,0};
		src[8]=new int[] {0,0,0,0,0,0,0,0,0};
		long start=System.currentTimeMillis();
		Matrix matrix=new Matrix(src);
		Phaser phaser=new Phaser();
//		executor.execute(new SquentialSolver(phaser,true,matrix));
		executor.execute(new SmartSolver(phaser,true,matrix));
//		List<Matrix> sdks=QueuedSolver.solve(matrix, true);
		phaser.awaitAdvance(0);
		long end=System.currentTimeMillis();
//		System.out.println("cost:"+(end-start)+"ms,"+sdks.size()+" SDK matrixs");
